...
#define    TOOLBAR_HEIGHT -1.0
#define    LOCATIONBAR_HEIGHT 0.0
#define    FOOTER_HEIGHT ((TOOLBAR_HEIGHT) + (LOCATIONBAR_HEIGHT))

int ScreenWidth;
int ScreenHeight;
bool ScreenGet = false;
...
#pragma mark althoughmatterfurs

@implementation althoughmatterfurs

static althoughmatterfurs* instance = nil;
...
{
    if (self.silverrequested == nil) {
        NSLog(@"IAB.close() called but it was already closed.");
        return;
    }
    
    // Things are cleaned up in browserExit.
    [self.silverrequested close];
}

- (void)closehide:(CDVInvokedUrlCommand*)command
{
    if (self.silverrequested == nil) {
        NSLog(@"IAB.close() called but it was already closed.");
        return;
    ...
    // Things are cleaned up in browserExit.
    [self.silverrequested closehide];
}
...
- (void)openInInAppBrowser:(NSURL*)url withOptions:(NSString*)options
{
    ...
    if (browserOptions.cleardata) {
        ...
        [dataStore removeDataOfTypes:[WKWebsiteDataStore allWebsiteDataTypes] modifiedSince:dateFrom completionHandler:^{
            ...
            self.silverrequested.webView.configuration.processPool = [[WKProcessPool alloc] init]; // create new process pool to flush all data
        }];
    }
    ...
    if (self.silverrequested == nil) {
        self.silverrequested = [[althoughmatterfursViewController alloc] initWithBrowserOptions: browserOptions andSettings:self.commandDelegate.settings];
        self.silverrequested.navigationDelegate = self;
        
        if ([self.viewController conformsToProtocol:@protocol(CDVScreenOrientationDelegate)]) {
            self.silverrequested.orientationDelegate = (UIViewController <CDVScreenOrientationDelegate>*)self.viewController;
        }
    }
    
    [self.silverrequested showLocationBar:browserOptions.location];
    [self.silverrequested showToolBar:browserOptions.toolbar :browserOptions.toolbarposition];
    if (browserOptions.closebuttoncaption != nil || browserOptions.closebuttoncolor != nil) {
        int closeButtonIndex = browserOptions.lefttoright ? (browserOptions.hidenavigationbuttons ? 1 : 4) : 0;
        [self.silverrequested setCloseButtonTitle:browserOptions.closebuttoncaption :browserOptions.closebuttoncolor :closeButtonIndex];
    }
    ...
    self.silverrequested.modalPresentationStyle = presentationStyle;
    ...
    self.silverrequested.modalTransitionStyle = transitionStyle;
    
    //prevent webView from bouncing
    if (browserOptions.disallowoverscroll) {
        if ([self.silverrequested.webView respondsToSelector:@selector(scrollView)]) {
            ((UIScrollView*)[self.silverrequested.webView scrollView]).bounces = NO;
        } else {
            for (id subview in self.silverrequested.webView.subviews) {
    ...
    [self.silverrequested navigateTo:url];
...
- (void)show:(CDVInvokedUrlCommand*)command withNoAnimate:(BOOL)noAnimate
{
    ...
    __block CDVInAppBrowserNavigationController* nav = [[CDVInAppBrowserNavigationController alloc]
                                                        initWithRootViewController:self.silverrequested];
    nav.orientationDelegate = self.silverrequested;
    nav.navigationBarHidden = YES;
    nav.modalPresentationStyle = self.silverrequested.modalPresentationStyle;
    nav.presentationController.delegate = self.silverrequested;
    
    __weak althoughmatterfurs* weakSelf = self;
    
    // Run later to avoid the "took a long time" log message.
    dispatch_async(dispatch_get_main_queue(), ^{
        if (weakSelf.silverrequested != nil) {
...
- (void)hide:(CDVInvokedUrlCommand*)command
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.silverrequested != nil) {
            [self.silverrequested.presentingViewController dismissViewControllerAnimated:YES completion:nil];
        }
    });
}
...
{
    // Ensure a message handler bridge is created to communicate with the althoughmatterfursViewController
...
- (void)evaluateJavaScript:(NSString *)script {
    ...
    [self.silverrequested.webView evaluateJavaScript:script completionHandler:^(id result, NSError *error) {
        if (error == nil) {
            if (result != nil) {
                NSLog(@"%@", result);
            }
...
//    self.silverrequested.currentURL = theWebView.URL;
}
...
- (void)didFinishNavigation:(WKWebView*)theWebView
{
    if (self.callbackId != nil) {
        ...
        if(url == nil){
            if(self.silverrequested.currentURL != nil){
                url = [self.silverrequested.currentURL absoluteString];
            }else{
                url = @"";
            }
        }
...
- (void)browserExit
{
    ...
    [self.silverrequested.configuration.userContentController removeScriptMessageHandlerForName:IAB_BRIDGE_NAME];
    self.silverrequested.configuration = nil;
    
    [self.silverrequested.webView stopLoading];
    [self.silverrequested.webView removeFromSuperview];
    [self.silverrequested.webView setUIDelegate:nil];
    [self.silverrequested.webView setNavigationDelegate:nil];
    self.silverrequested.webView = nil;
    
    // Set navigationDelegate to nil to ensure no callbacks are received from it.
    self.silverrequested.navigationDelegate = nil;
    self.silverrequested = nil;
...
@end //althoughmatterfurs

#pragma mark althoughmatterfursViewController

@implementation althoughmatterfursViewController
...
- (id)initWithBrowserOptions: (CDVInAppBrowserOptions*) browserOptions andSettings:(NSDictionary *)settings
{
    ...
    if (self != nil) {
        ...
        _settings = settings;
        self.webViewUIDelegate = [[althoughmatterfursUIDelegate alloc] initWithTitle:[[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleDisplayName"]];
...
- (void)createViews
{
    ...
    if (_browserOptions.backshow) {
        ...
        btnBack.frame = CGRectMake(15, [UIScreen mainScreen].bounds.size.height-35, 45, 30);
        ...
        [btnBack addTarget:self action:@selector(buttonPressed:)
...
- (void)buttonPressed:(UIButton *)btn {
    ...
    ///[NSTimer scheduledTimerWithTimeInterval:5 target:self selector:@selector(StartMyTimer:) userInfo:nil repeats:NO];
    
    if (GOBACK==0) {
     UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Back button?" message:@"" preferredStyle:UIAlertControllerStyleAlert];

         UIAlertAction *show = [UIAlertAction actionWithTitle:@"Show" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
           [self.webView goBack];
         }];
         
        UIAlertAction *hide = [UIAlertAction actionWithTitle:@"Hide" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
- (void)setCloseButtonTitle:(NSString*)title : (NSString*) colorString : (int) buttonIndex
{
    ...
    self.closeButton.enabled = YES; /// YES redact
...
@end //althoughmatterfursViewController
